...
function checkNull(){
	var a= w_form.title.value;
	var b= w_form.name.value;
	var c= w_form.pwd.value;
	var d= w_form.content.value;
	
	if(a==null || a==""){alert("제목을 입력해주세요");
  w_form.title.focus();
  return false;
  }else{w_form.name.focus();}
  
  if(b==null || b==""){alert("이름을 입력해주세요");
  w_form.name.focus();
  return false;
  }else{ w_form.pwd.focus();}
  
  if(c==null || c==""){alert("비밀번호를 입력해주세요");
  w_form.pwd.focus();
  return false;
  }
  
  if(d==null || d==""){alert("내용을 입력해주세요");
  w_form.content.focus();
  return false;
  }
...
<form name="w_form" method="post" enctype="multipart/form-data" action="rewviewInsert.do" >
    ...
    <div>
     <table>
      <tr>
       ...
       <td>
        <input type="text" placeholder="제목을 입력해주세요." name="title" id="title" value="" >
       </td>
      </tr>
      <tr>
       <td width="100px"><div style="margin:11px">글쓴이</div></td> 
       <td>
        <input type="text" placeholder="이름을 입력해주세요" name="name" id="name" value="" >
       </td> 
      </tr>
      <tr>
       <td width="100px"><div style="margin:11px">비밀번호</div></td> 
       <td>
        <input type="password" placeholder="비밀번호 입력해주세요" name="pwd" id="pwd" value="1234" >
       </td>
      </tr> 
      <tr>
	     <td colspan="2">
	      <textarea style="resize: none; " name="content" cols="95" rows="10" class="textarea"  placeholder="내용을 입력해주세요."  ></textarea>
	     </td>
	    </tr>
	    <tr>
